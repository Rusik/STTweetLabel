...
@property (nonatomic, strong) NSDictionary *attributesDefault;
...
- (void)setupLabel {
	...
	_attributesDefault = @{ NSForegroundColorAttributeName: self.textColor, NSFontAttributeName: [UIFont fontWithName:@"HelveticaNeue" size:14.0] };
...
- (void)updateText {
	...
	if (_cleanAttributedText) {
		...
		[attributedString setAttributes:[self attributes] range:NSMakeRange(0, _cleanText.length)];
	}
...
- (void)setDetectionBlock:(void (^)(STTweetHotWord hotWord, NSString *string, NSString *protocol, NSRange range))detectionBlock {
	if (detectionBlock) {
...
#define attributesOrDefault(attr) attr ? :_attributesDefault

- (NSDictionary *)attributes {
	return attributesOrDefault(_attributesText);
}

- (NSDictionary *)attributesForHotWord:(STTweetHotWord)hotWord {
	switch (hotWord) {
		case STTweetHandle :
			return attributesOrDefault(_attributesHandle);
			break;

		case STTweetHashtag:
			return attributesOrDefault(_attributesHashtag);
			break;

		case STTweetLink:
			return attributesOrDefault(_attributesLink);
			break;
